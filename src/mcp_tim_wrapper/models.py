...
class FlightWithEmissions(BaseModel):
    flight: Flight
    emissions_grams_per_pax: Optional[EmissionsGramsPerPax] = Field(None, alias='emissionsGramsPerPax')
...
class ComputeTypicalFlightEmissionsResponse(BaseModel):
    typical_flight_emissions: List[TypicalFlightEmission] = Field(alias='typicalFlightEmissions')
    model_version: ModelVersion = Field(alias='modelVersion')
