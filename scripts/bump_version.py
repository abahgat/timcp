...
def bump_version(current_version, new_version, file_path='pyproject.toml'):
    ...
    for line in lines:
        ...
        if inside_project and line.startswith('version = ') and f'"{current_version}"' in line:
    ...
    if not replaced:
        print(f'Error: Could not find version string "{current_version}" inside [project] block in {file_path}.')
        sys.exit(1)
